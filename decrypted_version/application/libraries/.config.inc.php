...
//echo dirname(__FILE__);
    ...
    ***********************************************************************/   
     function __autoload($className){//echo "<br>". $className;
        $filePath = str_replace('_', DIRECTORY_SEPARATOR, $className) . '.php';
        $includePaths = explode(PATH_SEPARATOR, get_include_path());//echo "<br>".var_dump($includePaths);
		 if(file_exists(APPPATH.'libraries/'. $filePath)){
			require_once APPPATH.'libraries/'. $filePath;
		 }
		//require_once(APPPATH.'libraries/Amazon/FPS/Client.php');
        foreach($includePaths as $includePath){//echo "<br>".$includePath . DIRECTORY_SEPARATOR . $filePath;
