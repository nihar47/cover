<?php
	...
	class Fb_connect {
		...
		function Fb_connect()
		{
			...
			if ($this->fbSession) {//var_dump($this->fbSession);
				try {
					//get information from the fb object
			    	$uid = $this->fb->getUser(); 
			    	$me = $this->fb->api('/'.$this->fbSession);
					//
					$this->friends = $this->fb->api('/'.$this->fbSession.'/friends');
					
					$this->user = $me;
					$this->user_id = $uid;
					
			    	$aResponse =$this->fb->api('/'.$this->fbSession, array(
   						 'fields' => 'picture',
   						 'type' => 'large'
					));
