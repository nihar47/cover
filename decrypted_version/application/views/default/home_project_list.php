...
<?php if($msg=='fail'){ ?>
<p style="color:#F00; text-align:center; font-size:14px; font-weight:bold;"><?php echo YOUR_DONATION_FAILED; ?></p><?php } 
?>

<?php if($msg=='cannot'){ ?>
<p style="color:#F00; text-align:center; font-size:14px; font-weight:bold;"><?php echo sprintf(YOU_CANNOT_DONATE_ON_THIS_PROJECT,$site_setting['maximum_donate_per_project']); ?></p><?php } 
?>

<?php if($msg=='not'){ ?>
<p style="color:#F00; text-align:center; font-size:14px; font-weight:bold;"><?php echo sprintf(YOU_CANNOT_CREATE_PROJECT,$site_setting['maximum_project_per_year']); ?></p><?php } 
?>
			...
			if(parseInt(myArray[1]) >= parseInt(limit)){	$('#last_msg_loader').html(''); }
				...
				if($(window).scrollTop() == $(document).height() - $(window).height()){				 
					$('#last_msg_loader').html('<div class="clear"></div><h3 class="discover" style="text-align:center;"><img src="<?php echo base_url();?>images/indicator.gif"></h3>');			  
					 setTimeout(function() { last_msg_funtion(); }, 20);
