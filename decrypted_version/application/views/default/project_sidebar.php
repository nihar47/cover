...
{
if($this->session->userdata('user_id') != $project['user_id']){?>
<tr>
	<td colspan="2" style="padding-top:5px;">
		<a id="various12" href="<?php echo site_url('message/send_message_project_detail/'.$project['project_id']);?>"  style=" color:#114A75; text-transform:capitalize; font-size:15px;"><?php echo CONTACT_ME;?></a>						 </td>
</tr>
<?php }
} else
{?>
	<tr>
	<td colspan="2" style="padding-top:5px;">
     		<a href="<?php echo site_url('home/login'.'/'.base64_encode(current_url())); ?>" style=" color:#114A75; text-transform:capitalize;  font-size:15px;"><?php echo CONTACT_ME;?></a>
		 </td>
</tr>
<?php }
}?>
...
{
if($this->session->userdata('user_id') != $project['user_id']){
		$chk_follower = project_follower_list($project['project_id'],$this->session->userdata('user_id'));
			 if($chk_follower){?>  
								<a id="followproject1" href="javascript:void(0)"  class="follow_btn" onclick="unfollowproject(<?php echo $project['project_id'];?>)"><?php echo UNFOLLOW_PROJECT;?></a>	
						<?php } 
							else{ 
						  ?>  
                          		<a id="followproject1" href="javascript:void(0)"  class="follow_btn" onclick="followproject(<?php echo $project['project_id'];?>)"><?php echo FOLLOW_PROJECT;?></a>
                          <?php }
						 ?>
 
<?php }else{?><a id="unfollowproject" href="<?php echo site_url('home');?>" class="follow_btn"><?php echo FOLLOW_PROJECT;?></a><?php }
}
else
{?>
	<a id="unfollowproject" href="<?php echo site_url('home/login'.'/'.base64_encode(current_url())); ?>" class="follow_btn"><?php echo FOLLOW_PROJECT;?></a></div>
<?php }
?>
</td>
</tr>
		...
		if($paypal_adptive_status=='1' ||  $paypal_normal_status=='1' || $amazon_status=='1' || $wallet_status=='1' || $credit_card_status==1)
				{
					 ...
					 if($project['active']==1) {
					 	...
					 	if(strtotime($dateg) >= strtotime(date('Y-m-d H:i:s'))) 
							{
							if($projemail==1)  { ?><a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="back_this" >Back This</a> 
								 
						 <?php } elseif($projtoken=='1') { ?><a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="back_this" ><?php echo BACK_THIS;?></a> 
                         <?php } elseif($wallet_status=='1'){ ?><a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="back_this" ><?php echo BACK_THIS;?></a> 
					 <?php } elseif($projcreditcard==1) { ?>  <a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="back_this" ><?php echo BACK_THIS;?></a> 
									<?php 
								 } else{
