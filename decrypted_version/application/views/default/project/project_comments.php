...
<div class="project_head"><?php echo anchor('projects/'.$one_project['url_project_title'].'/'.$one_project['project_id'],$one_project['project_title'],'class="project_title"');?>
  <p class="project_own"><?php echo BY; ?>&nbsp;<?php echo anchor('user/full_bio_data/'.$one_project['user_id'].'/'.$one_project['project_id'],$user_name.' '.substr($last_name,0,1),'id="profile_detail_popup"');?></p>
  <div class="clr"></div>
...
<div class="project_left">
  <div id="update_comment" class="update_comment">
    ...
    <a href="javascript://" id="showup" onclick="add_upd()"><?php echo ADD_COMMENTS; ?></a>
    ...
    <?php }}else{  ?>
    <span> <?php echo ONLY_BACKERS_CAN_POST_COMMENTSIF_YOU_HAVE_A_QUESTION; ?>,
    <?php if(get_authenticateUserID()!=""){ echo anchor('message/send_message_project_profile/'.$one_project['user_id'],ASK_THE_PROJECT_CREATOR,'id="ask_p_c1"'); }else{ echo anchor('home/login',ASK_THE_PROJECT_CREATOR); }?>
    . </span>
    <?php } ?>
  </div>
  <ul class="comment_listul">
    ...
    <li class="comment <?php if($upc['user_id']==$one_project['project_id']){ echo 'creator';} ?>">
      ...
      <a href="<?php echo site_url('member/'.$ud['user_id']); ?>"> <img src="http://graph.facebook.com/<?php echo $ud['fb_uid']; ?>/picture?type=large" class="cuserimg" /></a>
      <?php } else {
									 ?>
      ...
      <div class="update_cont">
        <h3>
          <?php if($upc['user_id']==$one_project['project_id']){?>
          <span class="creator"><?php echo CREATOR; ?></span>
          <?php } ?>
          <?php echo anchor('member/'.$ud['user_id'],$ud['user_name']." ".$ud['last_name'],'class="author"') ?> <span class="date"> <?php echo ABOUT; ?> <?php echo getDuration($upc['date_added']) ?></span> </h3>
    ...
    <?php 		 }	else{ ?>
    <h1><?php echo NO_COMMENT_AVAILABLE;?></h1>
    <?php }
				  ?>
  </ul>
        <div class="clr"></div>
