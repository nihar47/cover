  ...
  <script type="text/javascript">
    ...
    function Inviteme(fb_id) 
	{
		 ...
		 var response= FB.ui({
		  ...
		  description: "<?php echo ucfirst($user_info['user_name']).' '.ucfirst($user_info['last_name']);?> is on <?php echo ucfirst($site_setting['site_name']);?>, <?php echo FRIEND_TEXT.' '.ucfirst($site_setting['site_name']).'  '.TO_SEE;?> <?php echo ucfirst($user_info['user_name']);?>&acute;s campaigns.",
		  to: fb_id,
		   max_recipients:1,
  		  display: 'popup'
          },function(response){
