...
<div class="login_navl">
	...
	<span style=" font-family: 'Oswald',sans-serif;color:#222222;font-size:26px; text-align:left" ><?php echo MANAGE_WALLET_BELOW; ?></span>
...
<div class="inner_content" style=" margin-top:11px;padding:12px; ">
		...
		<?php /*	 <td align="right" valign="top"><?php echo anchor('wallet/add_wallet',ADD_WALLET,'style="font-weight:bold; color:#009900;font-size:13px !important;"');?></td>
			   <td width="10">&nbsp;|&nbsp;</td>
		*/ ?>	 <td align="right" valign="top"><?php echo anchor('wallet/my_withdraw',MY_WITHDRAWAL,'');?></td>
			 
			 <?php if($total_wallet_amount > intval($wallet_setting->wallet_minimum_amount)) { ?>
			  <td width="10">&nbsp;|&nbsp;</td>
			 <td align="right" valign="top"><?php echo anchor('wallet/withdraw_wallet',WITHDRAW_AMOUNT,'');?></td>
			 <?php } ?>
			 ...
			 $i=0;
				...
				<td align="left" valign="middle"><?php if($rs->credit>0) { echo DEBIT; } if($rs->debit>0) { echo CREDIT; } ?></td>
				<td align="left" valign="middle" style="padding-right:8px;"><?php if($rs->credit>0) { echo "-".set_currency($rs->credit); } if($rs->debit>0) { echo "+".set_currency($rs->debit); } ?>  </td>
				<!--<td align="left" valign="middle" style="padding-left:10px;"><?php //echo $rs->wallet_payee_email; ?></td>-->
				<td align="left" valign="middle"><?php echo $rs->wallet_transaction_id; ?></td>
				
				<td align="left" valign="middle"><?php if($rs->project_id=='' || $rs->project_id==0 || $rs->project_id=='0') { echo 'N/A';} else {
				...
				<td align="left" valign="middle"><?php if($rs->admin_status=='Review'){ ?><span style="color:#FF0000; font-weight:bold;"><?php echo $rs->admin_status; ?></span> <?php } if($rs->admin_status=='Confirm') { ?><span style="color:#009900; font-weight:bold;"><?php echo $rs->admin_status; ?> </span> <?php }  ?></td>

				<?php //if($wallet_setting->wallet_payment_type == '1'){ ?>
                	<td align="left" valign="middle"><?php if($rs->donate_status=='1'){ echo CONFIRM; } else{    
                ...
                <div style="position:relative;"><a href="javascript:\\" onmouseover="show_perk_detail('<?php echo $rs->id; ?>','<?php echo $i; ?>','show_view');" onmouseout="hide_perk_detail('<?php echo $rs->id; ?>','<?php echo $i; ?>','show_view');" style="display:inline-block;padding:3px;"><?php echo VIEW; ?></a>
                    <div style="position:absolute;top:25px;right:-8px; width:210px; border:1px solid #9FC8DA; padding:3px; display:none; background:#E3F0FD; text-align:left; z-index:1000; " id="show_view<?php echo $rs->id; ?><?php echo $i; ?>">
			 ...
			 <tr class="debit"><td colspan="<?php echo $colspan; ?>" height="35" align="center" valign="middle">
			 <div class="mess_cont_top_right"><?php echo $page_link; ?></div></td></tr>
