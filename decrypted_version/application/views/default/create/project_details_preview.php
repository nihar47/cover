...
<section>
	<div id="page_we">
		...
		<div class="step_cont_top">
				<h2><a  href="javascript://" class="project_title"><?php echo $project['project_title'];?></a></h2>
				<p>By&nbsp;<?php echo anchor('user/full_bio_data/'.$project['user_id'].'/'.$project['project_id'],getUserNamebyid($project['user_id']),'id="profile_detail_popup1" ');?></p>
...
-->		<?php 
            ...
            else
            {
                    ...
                    elseif(substr_count($project['video'],'vimeo')>0)
                    {
                        ...
                        echo '<iframe src="http://player.vimeo.com/video/'.$vid.'?title=0&byline=0&portrait=0" width="650" height="485"  frameborder="0"></iframe>';
             ...
             //http://www.youtube.com/watch?v=3iu6fhwtdl0
			 		...
			 		?>
					<img src="<?php echo base_url().'upload/thumb/no_video_available.jpg' ?>" width="650" height="485" style="border:1px solid #8B8B8B;" />	
			<?php 		
			 }
		?>		 
			...
			<!--	<a href="#inline1" class="embedbtn">Embed</a>-->
				<?php echo anchor('project/embed_popup/'.$project['project_id'],'Embed','class="embedbtn"'); ?>
				
                </div>
                <div class="project_reminder">
                	<h2><?php echo $project['project_title'];?></h2>
                    <!--<h1><img src="<?php echo base_url();?>images/calicon.png" alt="" class="icinimg"/><?php echo LAUNCHED; ?> : <?php echo date($site_setting['date_format'],strtotime($project['date_added']))?></h1>-->
                    <!--<h1><img src="<?php echo base_url();?>images/clockicon.png" alt="" class="icinimg"/><?php echo FUNDING_ENDS; ?> : <?php echo date($site_setting['date_format'],strtotime($project['end_date']))?></h1>-->
					 <!--<div id="follow_unfollow">
<?php 
if(get_authenticateUserID()>0)
{
if(get_authenticateUserID() != $project['user_id']){
		$chk_follower = project_follower_list($project['project_id'],get_authenticateUserID());
			 if($chk_follower){?>  
								<a id="followproject1" href="javascript:void(0)"  class="remindanchor1" onclick="unfollowproject(<?php echo $project['project_id'];?>)"><?php echo UNFOLLOW_PROJECT;?></a>	
						<?php } 
							else{ 
						  ?>  
                          		<a id="followproject1" href="javascript:void(0)"  class="remindanchor1" onclick="followproject(<?php echo $project['project_id'];?>)"><?php echo FOLLOW_PROJECT;?></a>
                          <?php }
						 ?>
...
?>
                ...
                <div class="faq">
                    ...
                    <p><strong style="color:#333333;"><?php echo HAVE_A_QUESTION; ?></strong> <?php echo IF_THE_INFO_ABOVE_DOESNT_HELP_YOU_CAN_ASK_THE_PROJECT_CREATOR_DIRECTLY; ?></p>
					 ...
					 if(get_authenticateUserID()!=$project['user_id']){
					 echo anchor('message/send_message_project_profile/'.$project['user_id'].'/'.$project['project_id'],ASK_A_QUESTION,'id="ask" class="askanchor"');
					 }else{ ?>
...
//echo strtotime($dategg);echo '='.strtotime(date('Y-m-d H:i:s'));
		...
		{					
			...
			//	echo 'paypal_adptive_status   '.$paypal_adptive_status;die;
				...
				if($project['user_id'] != get_authenticateUserID()){
					...
					if($paypal_adptive_status=='1' ||  $paypal_normal_status=='1' || $amazon_status=='1' || $wallet_status=='1' || $wallet_status=='0' || $credit_card_status==1)
					{
						 ...
						 if($project['active']==1) { 
							...
							if(strtotime($dateg) >= strtotime(date('Y-m-d H:i:s'))) 
							{
								 ...
								 if($projemail==1)  { ?>
								 <a href="<?php echo site_url('reward/'.$project['project_id']);?>" title="<?php echo ADD_FUND; ?>"><div class="back">
								...
								<?php } elseif($projtoken=='1') {?>
										...
										<a href="<?php echo site_url('reward/'.$project['project_id']);?>" title="<?php echo ADD_FUND; ?>">
														<div class="back">
														<h2><?php echo BACK_THIS_PROJECT;?></h2>
														<p><?php echo $site_setting['currency_symbol'] ?>1 <?php echo MINIMUM_PLEDGE; ?></p>
													 </div></a> 						 
                ...
                <?php  }
				 ...
				 ?> 
                     <p><?php echo THIS_PROJECT_WILL_BE_FUNDED_ON." ".date('F d , g:ia',strtotime($project['end_date'])); ?>. <?php echo HOW; ?> <a href="<?php echo site_url('home'); ?>">
					 <?php echo $site_setting['site_name'] .' '. KICKSTARTERCLONE_WORKS; ?></a>.</p>
                     
                     <div class="funding_period ">
                     <h3>Funding period </h3>
                     <p><?php echo date('M d,Y',strtotime($project['date_added'])); ?> - <?php echo date('M d,Y',strtotime($project['end_date'])); ?>(<?php echo $project['total_days']; ?>)</p>
                     </div>
...
<?php }  ?>
                    ...
                    <div class="contectinfo">
                       ...
                       <!-- <div class="cp"><?php echo get_city_name($user_city).",".get_state_name($user_state).",".get_country_name($user_country); ?></div>-->
                       <?php echo anchor('message/send_message_project_profile/'.$project['user_id'],CONTACT_ME,'id="contect_me"');?>
                    </div><div class="clr"></div>
                  <div class="divide_pro">  <a href="javascript://" class="userfb"><img src="<?php echo base_url(); ?>images/userfb.png" class="miniimg" alt="" /><?php echo HAS_NOT_CONNECTED_FACEBOOK; ?></a></div>
                ...
                <div class="visit_full_Profile">
				<?php echo anchor('user/full_bio_data/'.$project['user_id'].'/'.$project['project_id'],SEE_FULL_BIO,'id="profile_detail_popup" class="userfb"');?>
                </div>
	...
	?>
		...
		if($paypal_adptive_status=='1' ||  $paypal_normal_status=='1' || $amazon_status=='1' || $wallet_status=='1' || $credit_card_status==1)
					 	...
					 	if(strtotime($dateg) >= strtotime(date('Y-m-d H:i:s'))) 
							{
								if(get_authenticateUserID() > 0)
								{
								if($projemail==1)  { ?><a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="claimperk" ><?php echo CLAIM_THIS ?></a> 
									 
							 <?php } elseif($projtoken=='1') { ?><a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="claimperk" ><?php echo CLAIM_THIS ?></a> 
							 <?php } elseif($wallet_status=='1'){ ?><a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="claimperk" ><?php echo CLAIM_THIS ?></a> 
						 <?php } elseif($projcreditcard==1) { ?>  <a href="<?php echo site_url('reward/'.$project['project_id'].'/'.$perk['perk_id']); ?>" class="claimperk" ><?php echo CLAIM_THIS ?></a> 
										<?php 
									 } else{
            ...
            <div class="estimated"><?php echo EST_DELIVERY; ?> : <?php if($perk['perk_delivery_date']!="0000-00-00"){echo date($site_setting['date_format'],strtotime($perk['perk_delivery_date'])); }else {echo 'N/A';}?> </div>
						...
						<?php } }else{ ?>
						<li><h1 class="perkamt">There are no perks given by owner of this project</h1></li>
...
<div class="setp2btm">
	<div id="page_we">
        ...
        <?php echo anchor('start/launch_payment/'.$id,NEXT,'class="stepbtn"');?>
