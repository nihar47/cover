...
function show_perk_detail(id){
	document.getElementById('show'+id).style.display='block';
}	

function hide_perk_detail(id){
		...
		<span style="float:left;"><?php echo MY_WALLET; ?>( <?php	if($total_wallet_amount >=0){	echo set_currency($total_wallet_amount); } else{ echo '-'.set_currency(str_replace('-','',$total_wallet_amount)); }	?>)</span>
		 ...
		 <span style="float:right; height:35px;  font-size:12px;">
			 ...
			 <?php } ?>
			   ...
			   <?php echo $msg; die();if($msg!='') { ?><div style="clear:both; margin-top:20px;"></div> <div class="error" style="color:#FF0000;"><?php echo $msg;  ?></div><div style="clear:both; margin-top:20px;"></div>		<?php } 
			 ...
			 $i=0;
				...
				<td align="left" valign="middle"><?php if($rs->project_id=='' || $rs->project_id==0 || $rs->project_id=='0') { } else {
				...
				<?php if($wallet_setting->wallet_payment_type == '1'){ ?>
					...
					?>  <div style="position:relative;"><a href="javascript:\\" onmouseover="show_perk_detail('<?php echo $rs->id; ?>');" onmouseout="hide_perk_detail('<?php echo $rs->id; ?>');" style="display:inline-block;padding:3px;"><?php echo ONHOLD; ?></a>
                    <div style="position:absolute;top:25px;left:15px; width:200px; border:1px solid #9FC8DA; padding:3px; display:none; background:#E3F0FD; text-align:left; " id="show<?php echo $rs->id; ?>">
                	...
                	<td align="center" valign="middle"><?php if($rs->project_id > 0){	echo anchor('wallet/delete_wallet_donation/'.$rs->id,CANCEL); } ?></td>
                <?php  }  ?>
                </tr>
