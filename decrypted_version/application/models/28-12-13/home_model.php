...
class Home_model extends ROCKERS_Model {
	...
	public function register() {
		...
		$data = array("user_name" => $this->input->post("user_name"), "last_name" => $this->input->post("last_name"), "email" => $this->input->post("email"), "password" => $this->input->post("password"), "signup_ip" => $_SERVER["REMOTE_ADDR"], "active" => $active, "date_added" => date("Y-m-d H:i:s"), "image" => $this->input->post("image_name"), "tw_screen_name" => $this->input->post("tw_screen_name"), "fb_uid" => $this->input->post("fb_uid"), "fb_access_token" => $this->input->post("fb_access_token"), "tw_id" => $this->input->post("tw_id"), "tw_oauth_token" => $this->input->post("oauth_token"), "tw_oauth_token_secret" => $this->input->post("oauth_token_secret"), "confirm_key" => $confirm_key, "unique_code" => $unique_code, "reference_user_id" => $reference_user_id);
	...
	public function update_account() {
		...
		{
			$data = array("user_name" => $this->input->post("user_name"), "last_name" => $this->input->post("last_name"), "address" => $this->input->post("address"), "country" => $country, "city" => $city, "state" => $state, "zip_code" => $this->input->post("zip_code"), "gender" => $this->input->post("gender"), "user_about" => $this->input->post("user_about"), "user_occupation" => $this->input->post("user_occupation"), "user_interest" => $this->input->post("user_interest"), "user_skill" => $this->input->post("user_skill"), "profile_name" => $this->input->post("profile_name"));
		}
		 else 
		{
			$data = array("user_name" => $this->input->post("user_name"), "last_name" => $this->input->post("last_name"), "address" => $this->input->post("address"), "country" => $country, "city" => $city, "state" => $state, "zip_code" => $this->input->post("zip_code"), "gender" => $this->input->post("gender"), "user_about" => $this->input->post("user_about"), "user_occupation" => $this->input->post("user_occupation"), "user_interest" => $this->input->post("user_interest"), "user_skill" => $this->input->post("user_skill"));
		}
...
select * from project left join user on project.user_id=user.user_id  where project.active=1 and  (project.project_title like '%" . $text . "%' or user.user_name like '%" . $text . "%' or user.last_name like '%" . $text . "%' or project.project_summary like '%" . $text . "%') order by project.project_title asc limit 10";
	...
	public function get_total_my_back_project($uid = null) {
		...
		$query = $this->db->query("select project.* from project,transaction where project.project_id = transaction.project_id and transaction.user_id =" . $uid . " GROUP BY transaction.project_id");
		return $query->num_rows();
	}

	public function my_backed_project($user_id, $offset, $limit) {

		$query = $this->db->query("select project.*,transaction.transaction_date_time from project,transaction where project.project_id = transaction.project_id and transaction.user_id =" . $user_id . " GROUP BY transaction.project_id order by transaction.transaction_id LIMIT " . $limit . " OFFSET " . $offset);
		if ($query->num_rows() > 0)
		{
			return $query->result();
		}
