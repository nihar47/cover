...
<?php
		...
		$rowspan = SwmsHzrdsConsqs::Model()->countByAttributes(array(
				...
				foreach($hazards_consequences_model as $hazards_consequences_model_object) {
					...
					echo  '<td rowspan="'.$rowspan.'" align="left" valign="top" class="vertical_text">'.$swms_model->name.'</td>';
					$task_model_flag = 1;
					}
				
					if($hazards_consequences_model_flag == 0 ) {
					echo  '<td  rowspan="'.count($hazards_consequences_model).'" align="left" valign="top">'.$task_model_object->task.'</td>';
					...
					echo  '<strong>Hazards : </strong><br />'.$hazards_consequences_model_object->hazards;
					if(!empty($hazards_consequences_model_object->consequences))
					echo  '<br /><br /><strong>Consequences : </strong><br />'.$hazards_consequences_model_object->consequences;
					...
					echo  '<td align="left" valign="top">'.$risk_text.'</td>';
					echo  '<td align="left" valign="top">'.html_entity_decode($hazards_consequences_model_object->control_measures).'</td>';
					echo   '</tr>';
